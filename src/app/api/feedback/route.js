...
export async function POST(req) {
  try {
    ...
    const alwaysInvalid = Object.values(alwaysRequired).some(
      (r) => r === undefined || r < 1 || r > 5,
    );
    if (alwaysInvalid) {
      return new Response(
        JSON.stringify({
          error: "Missing required ratings. Ratings should be between 1 and 5.",
        }),
        { status: 400 },
      );
    }
    ...
    if (record.hasCar) {
      if (
        ...
        vehicleCondition > 5 ||
        !chauffeurProfessionalism ||
        chauffeurProfessionalism < 1 ||
        chauffeurProfessionalism > 5
      ) {
        return new Response(
          JSON.stringify({
            error:
              "Vehicle and chauffeur ratings are required for car service.",
          }),
          { status: 400 },
        );
      }
    ...
    if (record.hasBodyguard) {
      if (
        !bodyguardProfessionalism ||
        bodyguardProfessionalism < 1 ||
        bodyguardProfessionalism > 5
      ) {
        return new Response(
          JSON.stringify({
            error: "Bodyguard rating is required for bodyguard service.",
          }),
          { status: 400 },
        );
      }
    ...
    const updatedFeedback = await prisma.feedback.update({
      ...
      data: {
        ...
        ChauffeurProfessionalism: record.hasCar
          ? chauffeurProfessionalism
          : null,
        BodyguardProfessionalism: record.hasBodyguard
          ? bodyguardProfessionalism
          : null,
